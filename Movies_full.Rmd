...
I will build a model to predict the popularity of the movie among general audience and critics using MLR (Multiple Linear Regression) on the given dataset, as well as research on each predictor variable independently during EDA (if something interesting there to be found). Also I will try to explain popularity if model allows.
...
res_leaps<-as.data.frame(coef(leaps,which.max(sum$adjr2))) %>% pander()
```
...
res_critics <- as.data.frame(coef(leaps_cr,which.max(sum_cr$adjr2))) %>% pander()
```
...
res_caret <- as.data.frame(coef(step.model$finalModel, step.model$bestTune[[1]])) %>% pander()
```
...
data.frame(name = res_elastic@Dimnames[[1]][res_elastic@i + 1], coefficient = res_elastic@x) %>% pander()
```

...
-   **Generalizability** - random sample was taken and the number of observations is large, but here are considerations for generalizability:
    ...
    -   it looks like the ratings are relevant to the USA ( and probably Canada) population only - to be sure need to check the audience of IMDb and Rotten Tomatoes web services.
...
I will build a model to predict the popularity of the movies among general audiences and critics using MLR on the given dataset, as well as research on each predictor variable independently during the EDA (if something interesting is to be found). I will also try to explain popularity if the model allows.
...
In [Part 4: Modeling] I will build a few models for predicting general audience rating and select the best model based on predicting performance.

I will also build a model for predicting critic score - `critics_score` to compare it with general public score models.
...
Each variable will be explored separately in the EDA and decisions will be made accordingly.
...
We can see that the median `fin_score` is different among genres and some differences in the means are significant.
...
Let's use some silicon power =) There is not enough data and computational power to include interaction terms here, but let's compare with the manual model without interaction terms.
...
I will add one more model which will include interaction term between `runtime` and `title_type` (based on the previous `model.caret` ).
...
One of the methods of model selection is regularization. It is a great tool to deal with overfitting. I will check if it yields interesting results in my case to help with collinearities and influential points.
...
We can see that the results are not very impressive. Below are the coefficients of the best model
...
res_elastic <- coef(model.net,s = "lambda.min", exact=T)
data.frame(name = res_elastic@Dimnames[[1]][res_elastic@i + 1], coefficient = res_elastic@x) 
```
...
summary(model.robust)$adj.r.squared
```

Some promising results, I will compare testing results with the rest of the models

## Part 5: Prediction

Instead of one of the movies from 2016 I will use the test part of the data. I used movies to train models. Now I will use the remain part to predict and compare my models.
...
I did not include the elastic net model here because of poor performance on the test data previously. Looks like the best model for predicting `fin_score` is `model.interac` - the smallest MSE and the largest Efron's R\^2 and the worst is `model.leaps`. Model for predicting critics score `model.crit` has worse performance than any of the models for the general audience score. Which confirms that critics' opinions are less predictable.

#### Diagnostic of the best model

The best model seems to be the model based on the cross-validation selection technique with interaction terms `model.interac`.
...
There are the same problems as in the handmade model - heteroscedasticity, residual distribution is left-skewed, outliers, leverage points and non-linear trend in the residuals. Definitely more research is needed.
...
We can see that `title_type` and it's interaction term are under suspicion, but as the main purpose for the model is predicting power I will leave it as is.
...
ma_stud = rstudent(model.interac)
ols_plot_resid_stud(model.interac)
```
...
ols_plot_resid_lev(model.interac)
```
...
#number of points above 2 and  2/√(n)
dfbetas <- dfbeta(model.interac) 
df<-data.frame(
  length(dfbetas[abs(dfbetas)>2]),
  length(dfbetas[abs(dfbetas)>2/sqrt(length(ma_stud))])
)
names(df) <- c("Above 2","Above 2/√(n)") 
df
```

\
I've got quite a lot of suspicious points - not a perfect result. Looks like overfitting?
...
lag.plot(model.interac$residual, lags = 1, do.lines = F, labels = F)
```
...
As a measure of the effect size I will use standardized model parameters (coefficients). The standardized parameter is the difference between the means in SD of the response variable (`fin_score`).

More about standardized model parameters [<https://cran.r-project.org/web/packages/effectsize/vignettes/standardize_parameters.html>](https://cran.r-project.org/web/packages/effectsize/vignettes/standardize_parameters.html){.uri}

As it is similar to a **Cohen's *d*** (Cohen's d is the distance between the means in units of [**pooled SDs**](https://easystats.github.io/effectsize/reference/sd_pooled.html).) I will use this rule of thumb
...
We can see a decline in 1 SD from its base level - "Documentary" for both levels - "Feature Film" and "TV Movie"

-   `genre`

We can see a significant influence for many levels

-   `mpaa_rating` of a movie has a substantial influence on its popularity.
...
Month of a release has some influence too, but it is not a very strong effect.

-   `imdb_num_votes` and `best_pic_nom`

We can see that the number of voters and if a movie had been nominated are correlated with its popularity with the medium effect.

-   `title_type:runtime`

There is a very strong influence of the `runtime` variable (log of it if to be exact) on the "TV Movie" level of the `title_type` But if you remember - there are only 5 movies in this category - so it is probably just a noise.

-   And finally an enigmatic very small negative effect of the `best_pic_win` most likely due to collinearities between this variable and the `best_pic_nom`.
...
There are a few other minor problems: overfitting, violation of normality, non-linearity, influential points, collinearity and heteroscedasticity (full house =)?). I've tried some options like robust regression and elastic net regularization to deal with it, but it was not very helpful. I would like to try some ensemble methods later.

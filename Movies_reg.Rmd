...
-   **Generalizability** - random sample was taken and number of observations are large, but here are considerations for generalizability:
    ...
    -   looks like the ratings are relevant to the USA ( and probably Canada) population only - to be sure, I needs to check the audience of IMDb and Rotten Tomatoes web services.
...
movies %>% 
  ggplot() +
    aes(y = reorder(genre, fin_score, FUN = median), x = fin_score) +
    theme_light() +
    theme(axis.title.y=element_blank()) +
    geom_boxplot()
```

We can see that the median `fin_score` is different among genres - from lowest at Horror and Comedy to highest at Documentary.
...
movies %>% 
  ggplot() +
    aes(y = reorder(genre, critics_score, FUN = median), x = critics_score) +
    theme_light() +
    theme(axis.title.y=element_blank()) +
...
movies %>% 
  ggplot() +
    aes(y = reorder(mpaa_rating, fin_score, FUN = median), x = fin_score) +
    theme_light() +
    theme(axis.title.y=element_blank()) +
    geom_boxplot()
```

We can see a substantial difference in the medians among ratings
...
We can see that distribution of month is pretty fair and there are some difference in the medians of the scores
...
Residual plots look OK - fit is not perfect, but good enough, reasonably homoskedastic and some influential points.

#### ML - Exhaustive search

Let's use some silicon power =) There is not enough data and computational power to include interaction terms here, but let's compare with manual model selection without interacting terms.
...
We can see that the model is slightly different.
...
There is not enough information in the dataset - many predictor levels have too small numbers of observations.

There are too many influential points - I think that OLS is not the perfect option for this dataset. I would like to try something more robust (Robust regression - M-Estimator for example) or deal with collinearities.
